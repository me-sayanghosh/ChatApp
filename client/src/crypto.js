const RSA_KEY_STORAGE = 'chatapp:userRsaKeys';
const ROOM_KEY_STORAGE = 'chatapp:roomKeys';
...
export async function getUserKeyPair() {
  const stored = localStorage.getItem(RSA_KEY_STORAGE);
  if (stored) {
  ...
  localStorage.setItem(RSA_KEY_STORAGE, JSON.stringify({ publicKeyJwk, privateKeyJwk }));
  return keyPair;
}
...
export function clearAllCryptoKeys() {
  localStorage.removeItem(RSA_KEY_STORAGE);
